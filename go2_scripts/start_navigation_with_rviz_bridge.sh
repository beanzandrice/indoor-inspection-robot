...
: "${GO2_RVIZ_PROTOCOL:=binary}"
: "${GO2_RVIZ_TOPIC_ALLOWLIST:=default}"
: "${GO2_RVIZ_TOPIC_RATE_LIMITS:=}"
: "${GO2_RVIZ_DEBUG_ALL_TOPICS:=false}"
: "${GO2_RVIZ_QUEUE_CAPACITY:=32}"
: "${GO2_RVIZ_MAX_FRAME_BYTES:=16777216}"
: "${GO2_RVIZ_MAX_BUFFER_BYTES:=20971520}"
: "${GO2_RVIZ_WRITE_TIMEOUT:=2.0}"
...
ros2 launch go2_navigation go2_navigation_stack.launch.py \
  ...
  rviz_bridge_protocol:="${GO2_RVIZ_PROTOCOL}" \
  rviz_bridge_topic_allowlist:="${GO2_RVIZ_TOPIC_ALLOWLIST}" \
  rviz_bridge_topic_rate_limits:="${GO2_RVIZ_TOPIC_RATE_LIMITS}" \
  rviz_bridge_debug_all_topics:="${GO2_RVIZ_DEBUG_ALL_TOPICS}" \
  rviz_bridge_queue_capacity:="${GO2_RVIZ_QUEUE_CAPACITY}" \
  rviz_bridge_max_frame_bytes:="${GO2_RVIZ_MAX_FRAME_BYTES}" \
  rviz_bridge_max_buffer_bytes:="${GO2_RVIZ_MAX_BUFFER_BYTES}" \
  rviz_bridge_write_timeout:="${GO2_RVIZ_WRITE_TIMEOUT}" \
