...
tar -C "${ROOT_DIR}" -czf "${ARCHIVE}" go2_navigation go2_scripts
...
ssh -o StrictHostKeyChecking=accept-new \
  ...
  "set -euo pipefail
   ...
   tar -C /tmp/go2_navigation_upload -xzf /tmp/go2_navigation_project.tar.gz
   ...
   chmod +x '${REMOTE_PROJECT}/scripts/'*.sh '${REMOTE_PROJECT}/src/go2_navigation/go2_navigation/rviz_tcp_bridge.py'
   cd '${REMOTE_PROJECT}'
